...
def collect_links(driver, query):
    ...
    url = f'{base_url}{query.replace(" ", "+")}&hl=en"
    ...
    try:
        ...
        log.error("Timed out waiting for the results feed. Check your query.")
        return []
...
if __name__ == '__main__':
    ...
    try:
        if args.from_links:
            with open(args.from_links, 'r') as f:
                links = [line.strip() for line in f]
        elif args.url:
            links = collect_links(driver, args.url)
        else:
            query = args.query
            if not query:
                parser.print_help()
                sys.exit(1)
            links = collect_links(driver, query)

        if args.links_only:
            log.info(f"{len(links)} links collected. Exiting...")
        else:
            scrape_details(driver, links, f'{args.output}.csv')
    finally:
        driver.quit()
